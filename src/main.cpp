    ...
    while (window.isOpen() && state == State::InRoom)
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (exitBtn.clicked(event, window))
            {
                ...
                // Host 退出且房間變空 → 重置為 default（public）
                if (players.empty())
                    room.resetIfEmpty();
                else {
                    // 如果原本是 host 退出，交棒給下一位
                    // （這邊只是簡單讓 players[0] 變成新的 host）
                    // isHost=true/false 會在下次進來 InRoom 時重算
                }
        ...
        // Idle countdown：只有 NOT READY 時顯示
    ...
    while (window.isOpen())
    {
        switch (state)
        {
            ...
            case State::GameStart:
                // 目前只是 placeholder，你之後可以在這裡接真正的遊戲畫面
                window.clear();
                window.setView(uiView);
                window.draw(g_bgSprite);
                window.draw(g_bgOverlay);
